...
descr struc; Структура для описания декскриптора сегмента в таблице глобальных дескрипторов GDT
    limit  dw 0 ; Граница (биты 0..15)  - размер сегмента в байтах
    ...
    arrt_2 db 0 ; можно использовать этот сегмент.
...
pm_seg segment para public 'code' use32
    ...
    gdt             label byte
    gdt_null        descr <>
    gdt_data        descr <0FFFFh,0,0,10010010b,0cfh,0>           ;0cfh  - 11001111
    gdt_code16      descr <rm_seg_size-1,0,0,98h,0,0>       ;10011000 (98h) 
    gdt_code32      descr <pm_seg_size-1,0,0,98h,0CFh,0>    ;10011000  - сегмент кода
    gdt_data32      descr <pm_seg_size-1,0,0,92h,0CFh,0>    ;10010010  - сегмент данных и стека
    gdt_stack32     descr <stack_size-1,0,ы0,92h,0CFh,0>     ;10010010 (92h;;)
    gdt_screen      descr <4095,8000h,0Bh,92h,0,0>          ;10010010
    gdt_size=$-gdt
    ...
    ; Селекторы сегментов
    sel_data        equ 8           ;00001000
    sel_code16      equ 16
    sel_code32      equ 24
    sel_data32      equ 32
    sel_stack32     equ 40
    selector_screen     equ 48

    idt             label byte
    trap1           intr 13 dup (<0,sel_code32,0,8Fh,0>)
    trap13          intr <0,sel_code32,0,8Fh,0>
    trap2           intr 18 dup (<0,sel_code32,0,8Fh,0>)
    int_time        intr <0,sel_code32,0,8Eh,0>     ;первые 32 - исключения (8F)
    int_keyboard    intr <0,sel_code32,0,8Eh,0>     ;10001111 - 8Fh
    idt_size=$-idt
...
bs_pressed:
    ...
    ; Поставить курсор перед пробелом
    mov    screen_addr,ebx
...
end_of_memory:
    ...
    mov es, bx
    mov ecx, 8
    mov    ebp,5*160+2*(am_msg_len+7)+40
    mov    dh,attr2
cycle:
    mov    dl,al
    and    dl,0Fh
